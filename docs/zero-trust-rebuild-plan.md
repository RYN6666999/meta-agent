...
**閉環白話條文補充：**
- 每次修復或 closeout，必須自動產生一份知識文件，內容要有修復過程、驗證結果、負責人和期限，並自動檢查重複和資料完整性。
- 只要有人工介入，必須馬上驗證一次，隔一段時間再驗證一次，兩次都要寫進知識庫，否則不能關閉事件。
- 每次 PDCA 結束時，必須自動產生週報，並同步到 master-plan，讓下一輪任務直接用。
- 狀態同步修復後，必須自動驗證三份核心狀態一致，並寫進 lineage。
- CAPA 或系統性問題結案前，必須完成二次驗證和知識沉澱，否則不能結案。
